...
**Q: Why local instead of cloud APIs?**
- Privacy: Your research stays on your machine
- Cost: No API bills, unlimited usage
- Offline: Works without internet
- Customizable: Use any Ollama model
- Structured output: All agents return clean JSON

**Q: Can I use this without Ollama?**
- Not currently, but the architecture supports swapping `ollamaService.py`
- You could extend it to use other LLM providers (Claude, GPT, etc.)

**Q: How do I add my own documents?**
- Use the sidebar upload feature in the app
- Or manually copy JSON files to `data/knowledge/`
- Or call `index_knowledge_document()` in code

**Q: What if Ollama is slow?**
- Use a smaller model: `ollama pull orca-mini` (2.7B, fastest)
- Increase `OLLAMA_TIMEOUT` in `.env` if timeouts occur
- Increase your machine's available memory (minimum 4GB, recommended 8GB+)

**Q: Which model is best for structured JSON output?**
- All recommended models above work well
- **Best consistency:** mistral, llama2, neural-chat
- **Fastest:** orca-mini
- **Most capable:** llama2:13b (requires more VRAM)

**Q: Why is JSON sometimes malformed?**
- Models occasionally include explanation text with JSON
- The app automatically extracts valid JSON using multiple strategies
- If persistent, try a different model or increase timeout

**Q: Can I deploy this?**
- Yes! Use Streamlit Cloud, but be aware it needs a local Ollama instance
- Or self-host with Docker + Ollama
- Or use Streamlit + FastAPI server on a VPS with local Ollama

**Q: How do I check if my model produces good JSON?**
```bash
ollama generate mistral "Return only this JSON: {\"test\": true, \"value\": 42}"
```
If the output is clean JSON, the model is suitable.
...
### "Model not found"
- Run `ollama pull llama2` (or your chosen model)
- Check `OLLAMA_MODEL` in `.env` matches installed model
- List installed: `ollama list`

### Knowledge base is empty
- Upload documents using the sidebar
- Or place JSON files in `data/knowledge/`
- Check file permissions: `ls -la data/knowledge/`

### Slow responses or timeouts
- Use a smaller model: `ollama pull orca-mini`
- Reduce `max_sources` slider in the app
- Check your machine's available RAM: `free -h` (Linux/Mac) or Task Manager (Windows)
- Increase `OLLAMA_TIMEOUT` in `.env` (default: 300 seconds)

### JSON parsing errors in console
- This is logged but doesn't break the app (has fallbacks)
- To fix: try a different model (`mistral` or `llama2`)
- Reduce the size of input data being processed
- Check `[OllamaService]` logs for extraction strategy used

### App crashes or memory errors
- Reduce model size: `ollama pull orca-mini`
